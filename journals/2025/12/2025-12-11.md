---
tags:
  - type/journal
journal: Day
journal-date: 2025-12-11
---
# Journal 2025-12-11
## Tasks
```dataviewjs
const dateStr = "2025-12-11";
const targetDate = dv.date(dateStr);

// Fetch tasks once
const tasks = dv.pages().file.tasks.where(t =>
    !t.completed && (
        (t.due && t.due <= targetDate) ||
        (t.scheduled && t.scheduled <= targetDate) ||
        (t.start && t.start <= targetDate)
    )
);

if (tasks.length === 0) {
    dv.paragraph("No tasks for today! 🎉");
} else {
    // Group by tags
    for (let group of tasks.groupBy(t => t.tags.length > 0 ? t.tags[0] : "No Tag")) {
        dv.header(3, group.key);
        dv.taskList(group.rows, false);
    }
}
```
## 📝 Notes
- Watch the [IDEA Database](https://www.youtube.com/watch?v=nv8euZ2gOys) video.
- A new AI tool [OpenWebUI](https://openwebui.com/)
- Calisthenics [exercises](https://youtu.be/orV8GebjuaM?si=eGNIEDhnWLKJDzXJ)
- Interesting [desktop setup](https://www.youtube.com/watch?v=nQRInWf5IBE)

## 🍲 Menu
## 🔄 Progress on Goals
_Brief reflection on goal progress_
## 💭 Daily Reflection
_What went well? What could be improved?_

---
