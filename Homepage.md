...
for (let page of pages) {
    ...
    // Find the first "# Journal" heading
    const startIndex = linesArr.findIndex(l => l.trim().startsWith("# Journal"));
    let extracted = "";
    let hadMore = false;
    if (startIndex !== -1) {
        // Extract lines after the "# Journal" heading until the next H1 heading
        let section = [];
        for (let i = startIndex + 1; i < linesArr.length; i++) {
            const line = linesArr[i];
            const trimmed = line.trim();
            // Stop on next H1 heading (starts with "# " but not "##")
